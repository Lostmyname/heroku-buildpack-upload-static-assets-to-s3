...
try {
  ...
  var DEFAULT_REGION = getEnvVariable('AWS_DEFAULT_REGION');
  var BUCKET_NAME = getEnvVariable('AWS_STATIC_BUCKET_NAME');
  var SOURCE_DIRECTORY = getEnvVariable('AWS_STATIC_SOURCE_DIRECTORY');
  var STATIC_PREFIX = getEnvVariable('AWS_STATIC_PREFIX');
...
var dir = path.join(BUILD_DIR, SOURCE_DIRECTORY);
var STATIC_PATH = path.join(STATIC_PREFIX, new Date().toISOString().split('T')[0], SOURCE_VERSION);
    ...
    async.eachLimit(files, 16, function(file, callback) {
      ...
      function onUploadError(error) {
        ...
        fs.writeFileSync(
          ...
          'export STATIC_SERVER=' + BUCKET_NAME + '.s3.amazonaws.com' + '\n' +
          'export STATIC_SERVER_ACCELERATED=' + BUCKET_NAME + 's3-accelerate.amazonaws.com' + '\n' +
